...
//三个select的name
var q=["q_province","q_city","q_county"];
...
function qchange(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(q[i]).selectedIndex-1));
	};
	var ss=document.getElementById(q[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && document.getElementById(q[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
...
function q_init_area(){  //初始化函数
	for(i=0;i<q.length-1;i++){
	  document.getElementById(q[i]).onchange=new Function("qchange("+(i+1)+")");
	}
	qchange(0);
}
